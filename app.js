...
// HTML escape: previene XSS al renderizar datos de Firebase/localStorage en innerHTML
...
async function delInst(id) {
    ...
    try {
        ...
        if (inst) {
            if (inst.tipo === 'descarte' && Array.isArray(inst.items)) {
                for (const it of inst.items) await updStock(it.tipo, -it.cantidad);
            } else if (Array.isArray(inst.splitters)) {
                for (const s of inst.splitters) await updStock(s.tipo, -s.cantidad);
                if (inst.modulo) await updStock(inst.modulo, -1);
            }
        }
...
W.addSplEmp = function() {
    ...
    // Persist current notas/ref/fus inputs before re-render
...
W.enviarDescarte = async function() {
    ...
    try {
        await updStock(id, c); // descontar del stock (incrementa usada)
...
W.guardarEdit = async function() {
    ...
    // Preserve original record fields and any extra splitters beyond the first
    ...
    try {
        // Adjust stock for the edited (first) splitter: revert old, apply new
...
// Safer than embedding JSON.stringify in onclick: lookup by id
...
W.guardarConfig = function() {
    // Strip any HTML/control characters before storing
...
// ─── VOZ (en standby) ────────────────────────────
// La funcionalidad de voz está desactivada temporalmente.
// W.startVoz y W.stopVoz no hacen nada por ahora.
W.startVoz = function() {};
W.stopVoz = function() {};
...
function rInicio() {
    ...
    <div class="menu-cards">
        ${[['stock',I.box,'Ver stock','Inventario de splitters'],['calendario',I.cal,'Historial','Ver y editar días'],['material',I.plus,'Editar material','Añadir o quitar stock'],['descarte',I.trash,'Descarte','Material roto o no usable'],['avisos',I.bell,'Avisos y límites','Stock mínimo'],['informe',I.doc,'Informe del día','Resumen para compartir'],['config',I.gear,'Configuración','Equipo y matrícula']].map(([p,ic,t,s])=>`<button class="menu-card" onclick="ir('${p}')"><div class="menu-card-icon">${ic}</div><div class="menu-card-content"><p class="menu-card-titulo">${t}</p><p class="menu-card-subtitulo">${s}</p></div><span class="menu-card-flecha">${I.arrowS}</span></button>`).join('')}
    </div>
...
function rRegistrar() {
    ...
    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:var(--radio-sm);padding:14px;margin-bottom:14px;">
        ...
        ${conMod?`<div style="margin-top:12px;">
            ...
            ${modulo?`<div style="background:rgba(255,193,7,0.08);border:1px solid rgba(255,193,7,0.15);border-radius:8px;padding:10px 12px;margin-top:8px;"><span style="font-size:13px;color:var(--amarillo);font-weight:600;">✓ ${esc(lm(modulo))}</span></div>`:''}
        </div>`:''}
    </div>`;
...
function rInforme() {
    ...
    <span style="font-size:9px;font-weight:700;padding:3px 8px;border-radius:20px;text-transform:uppercase;background:${i.vertical==='Vertical propia'?'rgba(255,193,7,0.15)':'rgba(255,255,255,0.08)'};color:${i.vertical==='Vertical propia'?'var(--amarillo)':'rgba(255,255,255,0.6)'};">${i.vertical==='Vertical propia'?'Propia':'Demanda'}${i.cds?' · '+esc(i.cds)+' CDs':''}</span>
    ...
    const seccionPon = (ponKey, registros) => {
        ...
        return `<div style="margin:0 20px 16px;">${!esSin?`<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;"><div style="background:rgba(255,193,7,0.12);border:1px solid rgba(255,193,7,0.25);border-radius:20px;padding:5px 14px;"><span style="font-size:11px;font-weight:700;color:var(--amarillo);text-transform:uppercase;letter-spacing:1px;">PON ${esc(ponKey)}</span></div><div style="flex:1;height:1px;background:rgba(255,193,7,0.15);"></div><span style="font-size:11px;color:var(--gris-texto);">${registros.length} reg.</span></div>`:''}${pC.length?`<p style="font-size:10px;color:var(--gris-texto);text-transform:uppercase;letter-spacing:0.8px;margin:0 0 6px;font-weight:600;">CTOs</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:${pE.length?'10px':'0'};">${pC.map(cardCto).join('')}</div>`:''}${pE.length?`<p style="font-size:10px;color:var(--gris-texto);text-transform:uppercase;letter-spacing:0.8px;margin:0 0 6px;font-weight:600;">Empalmes</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">${pE.map(cardEmp).join('')}</div>`:''}</div>`;
    };
    ...
    ${contenido}
    ${descartes.length?`<div style="margin:8px 20px 16px;"><div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;"><div style="background:rgba(239,68,68,0.12);border:1px solid rgba(239,68,68,0.25);border-radius:20px;padding:5px 14px;"><span style="font-size:11px;font-weight:700;color:var(--rojo);text-transform:uppercase;letter-spacing:1px;">Descartes</span></div><div style="flex:1;height:1px;background:rgba(239,68,68,0.15);"></div><span style="font-size:11px;color:var(--gris-texto);">${descartes.length} reg.</span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">${descartes.map(d=>`<div style="background:rgba(239,68,68,0.06);border:1px solid rgba(239,68,68,0.18);border-radius:14px;padding:11px 12px;min-width:0;display:flex;flex-direction:column;gap:6px;">${(d.items||[]).map(it=>`<span style="font-size:13px;font-weight:600;color:#fff;font-family:'Poppins',sans-serif;">${esc(it.cat==='mod'?lm(it.tipo):ls(it.tipo))} ×${it.cantidad|0}</span>`).join('')}${d.motivo?`<span style="font-size:11px;color:var(--gris-texto);font-style:italic;">${esc(d.motivo)}</span>`:''}</div>`).join('')}</div></div>`:''}
...
function rDia() {
    ...
    const items = E.instDia, ed = E.instEdit;

    // ── Edit form (full-screen when editing a CTO) ──────────────────────────
    if (ed && ed.tipo === 'cto') {
        const i = ed;
        return `<div class="header"><button class="btn-atras" onclick="cancelEdit()">${I.back}</button><h2 class="titulo-pantalla">Editar CTO</h2></div>
        <div class="form">
            <div class="campo"><label class="campo-label">Splitter</label><select id="ed-spl" class="campo-select">${SPL.map(s=>`<option value="${s.id}" ${i.splitters?.[0]?.tipo===s.id?'selected':''}>${esc(s.l)}</option>`).join('')}</select></div>
            <div class="campo"><label class="campo-label">Cantidad</label><input type="number" id="ed-cant" class="campo-input" value="${(i.splitters?.[0]?.cantidad)|0||1}" min="1"></div>
            <div class="campo"><label class="campo-label">CTO</label><input type="text" id="ed-cto" class="campo-input" value="${esc(i.cto||'')}"></div>
            <div class="campo"><label class="campo-label">Notas</label><input type="text" id="ed-notas" class="campo-input" value="${esc(i.notas||'')}"></div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px;">
                <button onclick="guardarEdit()" style="background:var(--verde);color:#fff;border:none;border-radius:10px;padding:14px;font-family:Poppins,sans-serif;font-size:14px;font-weight:600;cursor:pointer;">Guardar</button>
                <button onclick="cancelEdit()" style="background:rgba(255,255,255,0.06);color:#fff;border:none;border-radius:10px;padding:14px;font-size:14px;font-weight:600;cursor:pointer;">Cancelar</button>
            </div>
            ${(i.splitters||[]).length>1?`<p style="font-size:11px;color:var(--gris-texto);margin:10px 0 0;">Esta CTO tiene ${i.splitters.length} splitters; sólo se editará el primero.</p>`:''}
        </div>`;
    }
    ...
    // ── Action buttons ───────────────────────────────────────────────────────
    const btnDel  = id => `<button onclick="delInst('${esc(id)}')" style="background:var(--rojo-bg);border:none;border-radius:8px;padding:5px 9px;font-size:11px;color:var(--rojo);font-weight:600;cursor:pointer;white-space:nowrap;">Eliminar</button>`;
    const btnEdit = id => `<button onclick="editInstHandler('${esc(id)}')" style="background:rgba(255,255,255,0.06);border:none;border-radius:8px;padding:5px 9px;font-size:11px;color:var(--gris-texto);font-weight:600;cursor:pointer;white-space:nowrap;">Editar</button>`;

    // ── Cards ────────────────────────────────────────────────────────────────
    const cardCto = i => `<div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:11px 12px;min-width:0;display:flex;flex-direction:column;gap:6px;">
        <div style="display:flex;justify-content:space-between;align-items:start;gap:4px;">
            <span style="font-size:15px;font-weight:700;font-family:'Poppins',sans-serif;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;">${esc(i.cto)}</span>
            <div style="display:flex;gap:4px;flex-shrink:0;">${btnEdit(i.id)}${btnDel(i.id)}</div>
        </div>
        ...
        <div style="display:flex;flex-wrap:wrap;gap:4px;">${(i.splitters||[]).map(s=>`<span style="font-size:10px;font-weight:600;background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.8);padding:3px 7px;border-radius:5px;border:1px solid rgba(255,255,255,0.08);">${esc(ls(s.tipo).replace('Splitter ',''))} ×${s.cantidad|0}</span>`).join('')}${i.modulo?`<span style="font-size:10px;font-weight:600;background:rgba(99,102,241,0.15);color:#a5b4fc;padding:3px 7px;border-radius:5px;border:1px solid rgba(99,102,241,0.2);">${esc(lm(i.modulo))}</span>`:''}</div>
        ${i.notas?`<span style="font-size:11px;color:var(--gris-texto);font-style:italic;">${esc(i.notas)}</span>`:''}
    </div>`;

    const cardEmp = i => `<div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:11px 12px;min-width:0;display:flex;flex-direction:column;gap:6px;">
        <div style="display:flex;justify-content:space-between;align-items:start;gap:4px;">
            <span style="font-size:15px;font-weight:700;font-family:'Poppins',sans-serif;color:#fff;flex:1;">${esc(i.referencia)}</span>
            ${btnDel(i.id)}
        </div>
        <span style="font-size:9px;font-weight:700;background:rgba(16,185,129,0.15);color:var(--verde);padding:3px 8px;border-radius:20px;display:inline-block;align-self:flex-start;">${esc(i.fusiones)} fusiones</span>
        <div style="display:flex;flex-wrap:wrap;gap:4px;">${i.splitters?.length?i.splitters.map(s=>`<span style="font-size:10px;font-weight:600;background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.8);padding:3px 7px;border-radius:5px;border:1px solid rgba(255,255,255,0.08);">${esc(ls(s.tipo).replace('Splitter ',''))} ×${s.cantidad|0}</span>`).join(''):'<span style="font-size:11px;color:var(--gris-texto);">Sin splitter</span>'}${i.modulo?`<span style="font-size:10px;font-weight:600;background:rgba(99,102,241,0.15);color:#a5b4fc;padding:3px 7px;border-radius:5px;border:1px solid rgba(99,102,241,0.2);">${esc(lm(i.modulo))}</span>`:''}</div>
        ${i.notas?`<span style="font-size:11px;color:var(--gris-texto);font-style:italic;">${esc(i.notas)}</span>`:''}
    </div>`;
    ...
    const contenido = items.length===0
        ? `<div class="vacio">${I.doc}<p>Sin registros</p></div>`
        : Object.entries(porPon).sort(([a],[b])=>a==='__sinpon__'?1:b==='__sinpon__'?-1:a.localeCompare(b)).map(([p,r])=>seccionPon(p,r)).join('');

    return `<div class="header"><button class="btn-atras" onclick="ir('calendario')">${I.back}</button><h2 class="titulo-pantalla" style="font-size:15px;text-transform:capitalize;">${esc(lab)}</h2></div>
    <div class="informe-resumen">
        ${ctos.length?`<div class="resumen-card"><span class="resumen-num">${ctos.length}</span><span class="resumen-lbl">CTO${ctos.length>1?'s':''}</span></div>`:''}
        ${empalmes.length?`<div class="resumen-card"><span class="resumen-num">${empalmes.length}</span><span class="resumen-lbl">Empalme${empalmes.length>1?'s':''}</span></div>`:''}
        ${totalFus?`<div class="resumen-card"><span class="resumen-num">${totalFus}</span><span class="resumen-lbl">Fusiones</span></div>`:''}
    ...
    ${contenido}
    ${descartes.length?`<div style="margin:8px 20px 16px;"><div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;"><div style="background:rgba(239,68,68,0.12);border:1px solid rgba(239,68,68,0.25);border-radius:20px;padding:5px 14px;"><span style="font-size:11px;font-weight:700;color:var(--rojo);text-transform:uppercase;letter-spacing:1px;">Descartes</span></div><div style="flex:1;height:1px;background:rgba(239,68,68,0.15);"></div><span style="font-size:11px;color:var(--gris-texto);">${descartes.length} reg.</span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">${descartes.map(d=>`<div style="background:rgba(239,68,68,0.06);border:1px solid rgba(239,68,68,0.18);border-radius:14px;padding:11px 12px;min-width:0;display:flex;flex-direction:column;gap:6px;"><div style="display:flex;justify-content:space-between;align-items:start;gap:4px;"><div>${(d.items||[]).map(it=>`<p style="font-size:13px;font-weight:600;margin:1px 0;color:#fff;font-family:'Poppins',sans-serif;">${esc(it.cat==='mod'?lm(it.tipo):ls(it.tipo))} ×${it.cantidad|0}</p>`).join('')}</div>${btnDel(d.id)}</div>${d.motivo?`<span style="font-size:11px;color:var(--gris-texto);font-style:italic;">${esc(d.motivo)}</span>`:''}</div>`).join('')}</div></div>`:''}
    ${splUsados.length?`<div style="margin:8px 20px 24px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:16px;"><p style="font-size:10px;color:var(--gris-texto);text-transform:uppercase;letter-spacing:0.8px;margin:0 0 12px;font-weight:600;">Splitters gastados</p>${splUsados.map(([tipo,cant])=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.05);"><span style="font-size:14px;font-weight:600;color:#fff;font-family:'Poppins',sans-serif;">· ${esc(ls(tipo))}</span><span style="font-size:14px;font-weight:700;color:var(--amarillo);font-family:'Poppins',sans-serif;">${cant} uds</span></div>`).join('')}</div>`:''}`;
...
// ─── RENDER & NAVIGATION ──────────────────────────
const PAGES = { inicio:rInicio, registrar:rRegistrar, confirmar:rConfirmar, stock:rStock, calendario:rCalendario, dia:rDia, material:rMaterial, descarte:rDescarte, avisos:rAvisos, informe:rInforme, pedido:rPedido, config:rConfig };
...
async function ir(p) {
    E.pantalla = p;
    if (['stock','pedido','inicio','material','avisos','descarte'].includes(p)) await loadStock();
    ...
    if (p==='descarte') E.fDes = {tipo:'splitter',splitter:'',modulo:'',cantidad:'',motivo:''};
