...
function rInforme() {
    ...
    const totalSpl = items.reduce((a,i)=>a+(i.splitters?.reduce((b,s)=>b+(s.cantidad||0),0)||0),0);
    const totalFus = items.reduce((a,i)=>a+(parseInt(i.fusiones)||0),0);
    const totalCds = ctos.reduce((a,c)=>a+(parseInt(c.cds)||0),0);

    const porPon = {};
    items.forEach(i => { const p=i.pon||'__sinpon__'; if(!porPon[p]) porPon[p]=[]; porPon[p].push(i); });
...
// ─── RENDER & NAVIGATION ──────────────────────────
const PAGES = { inicio:rInicio, registrar:rRegistrar, confirmar:rConfirmar, stock:rStock, calendario:rCalendario, dia:rDia, material:rMaterial, avisos:rAvisos, informe:rInforme, pedido:rPedido, config:rConfig };
