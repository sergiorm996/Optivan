...
function rDia() {
    ...
    const lista = inst.length===0?`<div class="vacio">${I.doc}<p>Sin registros</p></div>`:inst.map(i=>{
        const isEmp = i.tipo==='empalme', isDes = i.tipo==='descarte', isEd = ed&&ed.id===i.id;
        const idEsc = esc(i.id);
        if (isDes) {
            const items = i.items || [];
            return `<div class="informe-item" style="border-left:3px solid var(--rojo);"><div style="display:flex;justify-content:space-between;align-items:start;"><div><div style="background:var(--rojo);border-radius:5px;padding:3px 8px;display:inline-block;margin-bottom:5px;"><span style="font-size:9px;font-weight:700;color:#fff;text-transform:uppercase;">DESCARTE</span></div>${items.map(it=>`<p style="font-size:15px;font-weight:600;margin:2px 0;color:#fff;font-family:Poppins,sans-serif;">${esc(it.cat==='mod'?lm(it.tipo):ls(it.tipo))} ×${it.cantidad|0}</p>`).join('')}${i.motivo?`<p style="font-size:12px;color:var(--gris-texto);margin:4px 0 0;font-style:italic;">${esc(i.motivo)}</p>`:''}</div><button onclick="delInst('${idEsc}')" style="background:var(--rojo-bg);border:none;border-radius:8px;padding:8px 12px;font-size:12px;color:var(--rojo);font-weight:600;cursor:pointer;">Eliminar</button></div></div>`;
        }
